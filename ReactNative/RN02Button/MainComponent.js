...
class MainComponent extends Component{
    ...
    // 멤버변수는 가급적 생성자 함수 안에서 this 키워드로 생성 및 지정
    constructor(){
    ...
    changeTextByClickBtn(){
        ...
        // 3-2) 멤버변수 값 변경
        // 일반 함수 안에서의 this는 함수(객체) 자체를 의미
        // 그래서 this.text는 error
    ...
    changeTextByAnonymous= function() {
        ...
        // 익명함수도 this는 본인 객체(함수)를 의마함
        // 그래서 this.text가 MainComponent의 text 변수가 아니었음
        this.text="Good!";
    }

    //3-4) 두꺼운 화살표함수로 만드는 방법
    changeTextByArrow= ()=>{
    ...
    // 그래서 Component 클래스 안에 아주 특별한 멤버변수를 이미 만들어 놓았음
...
// 1)의 실습에 사용할 버튼 클릭 콜백함수 : 1-1) 선언적함수
...
// 객체의 버튼 이벤트는 그 객체 안에 처리 메소드가 있는 것이 좋음
